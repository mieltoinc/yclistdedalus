...
import { YCListServer } from './server.js';
import { runStdioTransport, startHttpTransport } from './transport/index.js';

/**
 * Main entry point for the YC Lists MCP Server
 * 
...
async function main() {
    try {
        ...
        if (shouldUseHttp) {
            ...
            const server = new YCListServer();
            await runStdioTransport(server.getServer());
        }
    } catch (error) {
        console.error("Fatal error running YC Lists server:", error);
        process.exit(1);
    }
}
