...
async function createNewSession(
    ...
    const transport = new StreamableHTTPServerTransport({
        ...
        onsessioninitialized: (sessionId) => {
            ...
            console.log('New YC Lists session created:', sessionId);
        }
    });
    ...
    transport.onclose = () => {
        if (transport.sessionId) {
            ...
            console.log('YC Lists session closed:', transport.sessionId);
        }
    };
...
function logServerStart(config: Config): void {
    ...
    console.log(`YC Lists MCP Server listening on ${displayUrl}`);
